...
non-employed people. However, from an average contribution perspective,
...
il.contr_by_party <- summarize(party_group,
...
il.contr_by_party
il.contr_by_party$party <- ordered(il.contr_by_party$party, 
                                   levels = c('democrat', 'republican', 'others'))

ggplot(data = il.contr_by_party, aes(x = party, y = n, fill = party)) +
  geom_bar(stat = 'identity') +
  geom_text(stat = 'identity', aes(label = n),
            data = il.contr_by_party, vjust = -0.4) +
  xlab('Party') +
